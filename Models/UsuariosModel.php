...
class UsuariosModel extends Query{
    ...
    public function getUser_api($user)
    {
        $ch = curl_init();
        curl_setopt($ch, CURLOPT_URL, urlapiMysql."/usuarios/".$user);
        curl_setopt($ch, CURLOPT_RETURNTRANSFER, true);
        $res = curl_exec($ch);
        curl_close($ch);   
        return $res;
    }
    public function getHash_api($user)
    {
        $ch = curl_init();
    ...
    public function ExtraerDatos_api($empresa)
    {
        $ch = curl_init();
    ...
    public function conectar_api_jhon_vena($usuario, $clave)
    {
        $sql = "SELECT * FROM usuarios WHERE usuario = '$usuario' AND clave = '$clave' AND estado = 1";
        $data = $this->select($sql);
        return $data;
